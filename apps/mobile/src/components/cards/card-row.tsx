...
export function CardRow({
  ...
  return (
    <Pressable onPress={onPress} style={{ flexDirection: 'row', alignItems: 'center', gap: 10, paddingVertical: 8, paddingHorizontal: 4 }}>
      {grad ? (
      ...
      {card.official ? (
        <Pressable onPress={onPress} style={{ flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 9999, backgroundColor: t.dark ? '#0F2A18' : idnTokens.greenSoft }}>
          <Text style={{ fontSize: 11, fontWeight: '600', color: idnTokens.green }}>Voir</Text>
          <Icon name="arrow" size={12} color={idnTokens.green} />
        </Pressable>
      ) : (
        <View style={{ flexDirection: 'row', gap: 4 }}>
          <Pressable onPress={onEdit} style={{ width: 30, height: 30, borderRadius: 8, borderWidth: 1, borderColor: t.borderSoft, alignItems: 'center', justifyContent: 'center' }}>
            <Icon name="edit" size={16} color={t.muted} />
          </Pressable>
          <Pressable
            onPress={onToggleFeatured}
            disabled={!featuredAction && atMax}
            style={{ width: 30, height: 30, borderRadius: 8, borderWidth: 1, borderColor: t.borderSoft, alignItems: 'center', justifyContent: 'center', opacity: !featuredAction && atMax ? 0.3 : 1 }}
          >
            <Icon name={featuredAction ? 'eyeOff' : 'eye'} size={16} color={t.muted} />
          </Pressable>
          <Pressable onPress={onDelete} style={{ width: 30, height: 30, borderRadius: 8, borderWidth: 1, borderColor: t.borderSoft, alignItems: 'center', justifyContent: 'center' }}>
