...
export default function IBoiteHome() {
  ...
  const [tab, setTab] = useState<IBoiteTab>("emails")
    ...
    return (
      ...
      >
        <Text style={{ color: t.muted, fontSize: 13 }}>
          Connectez-vous pour accéder à iBoîte.
        </Text>
      </View>
    )
  }
  ...
  if (accounts.length === 0 || !selectedAccountId) {
    return (
      ...
      >
        ...
        >
          Aucun compte iBoîte. Terminez votre inscription pour activer votre
          adresse souveraine.
        </Text>
      </View>
    )
  ...
  return (
    <View style={{ flex: 1, backgroundColor: t.bg, paddingTop: insets.top }}>
      ...
      <View style={{ paddingHorizontal: 16 }}>
        <AccountPill
          acc={accUi}
          onPress={() => router.push("/iboite/accounts" as never)}
        />
      </View>
      {tab !== "emails" ? (
        <AddressStrip
      ...
      {tab === "colis" ? (
        <ColisTab accountId={account._id as never} qr={accUi.addr.qr} />
      ) : null}
      {tab === "emails" ? (
        <EmailsTab
          accountId={account._id as never}
      ...
      >
        ...
        {letters.length === 0 ? (
          <View style={{ paddingVertical: 40, alignItems: "center" }}>
            ...
            <Text style={{ fontSize: 12, color: t.muted, marginTop: 10 }}>
                ...
                <View style={{ flex: 1, minWidth: 0 }}>
                  ...
                  >
                    <Text style={{ fontSize: 10, color: t.muted }}>
                      ...
                      {l.dueAt
                        ? ` · Réponse avant ${new Date(l.dueAt).toLocaleDateString("fr-FR", { day: "2-digit", month: "short" })}`
                        : ""}
                    </Text>
                    {!l.isRead ? (
                      <View
...
function ColisTab({ accountId, qr }: { accountId: string; qr: string }) {
  const t = useIdnTheme()
  const insets = useSafeAreaInsets()
  const data = useQuery(api.iboite.packages.listMine, {
  ...
  return (
    ...
    >
      <View
        style={{ flexDirection: "row", gap: 8, marginBottom: 14, marginTop: 8 }}
      >
        <View
          style={{
            flex: 1,
            backgroundColor: t.dark ? "#3A2D14" : "#FEF3C7",
            borderWidth: 1,
            borderColor: t.dark ? "#5A4626" : "#FDE68A",
            borderRadius: 12,
            padding: 12,
          }}
        >
        ...
        <View
          style={{
            ...
            borderWidth: 1,
            borderColor: t.dark ? "#1B3F5A" : "#BFD9F7",
            borderRadius: 12,
            padding: 12,
          }}
        >
          <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
            ...
            <Text
              style={{ fontSize: 22, fontWeight: "700", color: idnTokens.blue }}
            >
              {data.transit}
            </Text>
          </View>
        ...
        data.items.map((p) => {
          ...
          return (
            ...
            >
              ...
              >
                <Icon
                  name={avail ? "package" : "truck"}
                  size={18}
                  color={avail ? "#92400e" : idnTokens.blue}
                />
              </View>
              <View style={{ flex: 1 }}>
              ...
              <View style={{ alignItems: "flex-end", gap: 4 }}>
                <View
                  style={{
                    ...
                    paddingVertical: 3,
                    borderRadius: 9999,
                    backgroundColor: avail ? "#FEF3C7" : idnTokens.blueSoft,
                  }}
                >
                  <Text
                    style={{
                ...
                {p.estimatedDeliveryAt ? (
                  <Text style={{ fontSize: 10, color: t.muted }}>
                    Arrivée{" "}
                    {new Date(p.estimatedDeliveryAt).toLocaleDateString(
                      "fr-FR",
      ...
      >
        ...
        >
          ...
          <TextInput
            ...
            style={{ flex: 1, fontSize: 14, color: t.ink, paddingVertical: 0 }}
          />
        </View>
        <ScrollView
          horizontal
          ...
          visibleEmails.map((e, i) => (
            <Pressable
              ...
              style={{
                ...
                paddingVertical: 12,
                paddingHorizontal: 4,
                borderBottomWidth: i === visibleEmails.length - 1 ? 0 : 1,
            ...
            >
              <Pressable
                onPress={async () => {
                  try {
                    await toggleStar({ messageId: e._id as never })
                  } catch {
                    /* ignore */
                  }
                }}
              ...
              >
                <Icon
                  name={e.isStarred ? "star" : "starO"}
                  size={16}
                  color={e.isStarred ? idnTokens.yellow : t.mutedSoft}
                />
              </Pressable>
              <View
                style={{
                  ...
                  justifyContent: "center",
                  backgroundColor:
                    e.senderKind === "admin" ? "#3b82f6" : "#10b981",
                }}
              >
                <Icon
                  name={e.senderKind === "admin" ? "building" : "user"}
                  size={18}
                  color="#fff"
                />
              </View>
              <View style={{ flex: 1, minWidth: 0 }}>
                ...
                >
                  <Text
                    ...
                    style={{
                      flex: 1,
                      fontSize: 13,
                      fontWeight: e.isRead ? "500" : "700",
                      color: e.isRead ? t.ink2 : t.ink,
                    }}
                  >
                    {e.senderName}
                  </Text>
                ...
                <Text
                  ...
                  style={{
                    fontSize: 12,
                    color: e.isRead ? t.muted : t.ink,
                    fontWeight: e.isRead ? "500" : "600",
                    marginTop: 2,
                  }}
                >
                  {e.subject}
                ...
                >
                  <Text
                    numberOfLines={1}
                    style={{ flex: 1, fontSize: 11, color: t.muted }}
                  >
                    {e.preview}
                  </Text>
                  {e.hasAttachment ? (
                    <Icon name="paper" size={12} color={t.muted} />
                  ) : null}
                </View>
              </View>
            </Pressable>
