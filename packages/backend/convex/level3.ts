...
import { requireAuth, requireController, requireVerifiedAuth } from "./lib/auth"

const ACTIVE_STATUSES = ["waiting_controller", "claimed", "in_interview"] as const
    ...
    return await Promise.all(
      rows.map(async (row) => {
        const profile = await ctx.db
          ...
          .unique()
        const name = [profile?.pivot?.firstName, profile?.pivot?.lastName].filter(Boolean).join(" ")
        return {
          _id: row._id,
...
export const myCurrent = query({
  ...
  handler: async (ctx) => {
    ...
    const recent = await ctx.db
      .query("level3Verification")
      .withIndex("by_controllerId", (q) => q.eq("controllerId", controller.userId))
      .order("desc")
      .take(10)
...
export const claim = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const current = await ctx.db
      ...
      .order("desc")
      .take(10)
    if (current.some((row) => row.status === "claimed" || row.status === "in_interview")) {
      throw new ConvexError({
        code: "CURRENT_INTERVIEW_EXISTS",
        message: "Terminez votre entretien en cours avant d'en prendre un autre.",
      })
    }
    const now = Date.now()
    await ctx.db.patch(args.verificationId, {
...
export const _getJoinContext = internalQuery({
  ...
  handler: async (ctx, args) => {
    const me = await requireAuth(ctx)
    const verification = await ctx.db.get(args.verificationId)
    if (!verification || !["claimed", "in_interview"].includes(verification.status)) {
      throw new ConvexError({
    ...
    const isController =
      verification.controllerId === me.userId && me.roles.includes("identity_controller")
    ...
    const role: "citizen" | "controller" = isController ? "controller" : "citizen"
    const profile = await ctx.db
      .query("userProfile")
      .withIndex("by_userId", (q) => q.eq("userId", me.userId))
      .unique()
    ...
    return {
      ...
      participantName: name || (role === "controller" ? "Contrôleur" : "Citoyen"),
      role,
    }
  },
})
