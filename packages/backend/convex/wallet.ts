...
// ─────────────────────────────────────────────────────────────────────────
// Internal — seed appelé depuis onboarding.selectProfile
// ─────────────────────────────────────────────────────────────────────────

/**
 * Catalogue par défaut des cartes du citoyen (verbatim spec §1.6).
 * Les 6 premières sont mises en avant (`featured=true`). La carte consulaire
 * est créée mais non featured (l'utilisateur peut l'activer ensuite).
 */
const DEFAULT_CARDS: Array<{
  type: (typeof WALLET_CARD_TYPES)[number]
  name: string
  subtitle: string
  gradient: string
  iconKey: string
...
}> = [
  {
    ...
    featured: true,
    data: { nom: "", numero: "", validite: "" },
    backData: { naissance: "", lieu: "", sexe: "", taille: "" },
  },
  {
    type: "driving",
    ...
    isOfficialStyle: false,
    featured: true,
    data: { nom: "", numero: "", categories: "" },
    backData: { delivrance: "", prefecture: "", points: "" },
  },
  {
    type: "transport",
    ...
    isOfficialStyle: false,
    featured: true,
    data: { zone: "Toutes zones", numero: "", validite: "" },
    backData: { type: "Abonnement Mensuel", solde: "" },
  },
  {
    type: "health",
    name: "CNAMGS",
    ...
    featured: true,
    data: { regime: "", numero: "", validite: "" },
    backData: { employeur: "", couverture: "" },
  },
  {
    type: "bank",
    name: "Carte Bancaire",
    ...
    isOfficialStyle: false,
    featured: true,
    data: { numero: "", titulaire: "", expiration: "" },
    backData: { cvv: "", plafond: "" },
  },
  {
    type: "business",
    ...
    isOfficialStyle: false,
    featured: true,
    data: { titre: "", entreprise: "" },
    backData: { email: "", tel: "", site: "" },
  },
  {
    type: "consular",
    ...
    isOfficialStyle: false,
    featured: false,
    data: { numero: "", nip: "", validite: "" },
    backData: { consulat: "", zone: "" },
  },
]

export const seedDefaultsForUser = rawInternalMutation({
  args: { userId: v.string() },
  returns: v.null(),
  handler: async (ctx, args) => {
    // Idempotent : si une carte existe déjà pour ce user, on ne re-seed pas.
    ...
    for (let i = 0; i < DEFAULT_CARDS.length; i++) {
      const c = DEFAULT_CARDS[i]!
      await ctx.db.insert("walletCard", {
        userId: args.userId,
        type: c.type,
        name: c.name,
